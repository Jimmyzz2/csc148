...
class LinkedList:
    ...
    def insert_list(self, other: LinkedList, i):
        """
        >>> l = LinkedList([0,1,2,3,4])
        >>> o = LinkedList([100,101])
        >>> l.insert_list(o, 3)
        >>> print(l)
        [0 -> 1 -> 2 -> 3 -> 100 -> 101 -> 4]
        >>> l = LinkedList([0,1,2,3,4])
    ...
    def bisect(self, i):
        ...
        >>> str(linky1)
        '[1 -> 2 -> 3 -> 4]'
        >>> linky1.bisect(3).item
        4
        >>> str(linky1)
        '[1 -> 2 -> 3]'
        >>> linky1 = LinkedList([1,2,3,4])
        >>> linky1.bisect(0).item
        1
        >>> str(linky1)
        '[]'
        """
        ...
        index = 0
        curr = self._first
        while curr.next is not None and index < i- 1:
            curr = curr.next
            index += 1
        if curr is None or (curr.next is None and index <= i-1) or i <0:
            raise IndexError
        record = curr.next
        curr.next = None
        return record
