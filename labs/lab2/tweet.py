...
# Which class should we put it into? Once you've decided, move it.
    def retweet(self, tweet: Tweet, new_date: date) -> Tweet:
        ...
        >>> user1 = User("Diane", "i am funny")
        >>> t1 = Tweet('David', date(2017, 8, 19), 'David is so cool!')
        >>> ret = user1.retweet(t1, date(2017, 8, 20))
        >>> ret.content
        'David is so cool!'
        >>> user1.tweets[0].content
        'David is so cool!'
        >>> user1.userid
        'Diane'
        >>> user1.tweets[0].created_at
        datetime.date(2017, 8, 20)
        """
        re_tweet = Tweet(self.userid, new_date, tweet.content)
        self.tweets.append(re_tweet)
        return re_tweet
