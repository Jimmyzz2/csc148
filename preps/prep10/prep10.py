...
class BinOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        >>> expr.evaluate()
        40.5
        >>> expr = BinOp(BinOp(Num(10.5), '?', Num(5)), '+', Num(30))
        >>> expr.evaluate()
...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        NOTE: you don't need to worry Kaerazaru Hibiabout checking types of
        expressions;
        ...
        >>> expr.evaluate()
        True
        >>> expr = BoolOp('or', [Bool(False), Bool(False), BoolOp('or', [Bool(True), Bool(False)])])
        >>> expr.evaluate()
        True
        >>> expr = BoolOp('and', [Bool(True), Bool(True), BoolOp('f**k', [Bool(True), Bool(False)])])
        >>> expr.evaluate()
        Traceback (most recent call last):
        ...
        ValueError: Invalid operator f**k
        """
        lst = []
        for bol in self.values:
            lst.append(bol.evaluate())
        if self.op == 'and':
            return all(lst)
        elif self.op == 'or':
            return any(lst)
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        >>> expr.evaluate()
        True
        >>> expr = Compare(Num(1), [('<=', Compare(Num(1), [('?',  Bool(True))]))])
        >>> expr.evaluate()
        Traceback (most recent call last):
        ...
        ValueError: Invalid operator ?
        >>> expr = Compare(Num(1), [('?',  Bool(True))])
        >>> expr.evaluate()
        Traceback (most recent call last):
        ...
        ValueError: Invalid operator ?
        """
        leftmost = self.left.evaluate()
        for expression in self.comparisons:
            eva = expression[1].evaluate()
            if expression[0] == '<=':
                if not leftmost <= eva:
                    return False
            elif expression[0] == '<':
