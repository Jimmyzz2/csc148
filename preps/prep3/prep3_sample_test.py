...
def test_total_pay_basic2() -> None:
    e = HourlyEmployee(14, 'Gilbert the cat', 1200.0, 50.0)
    e.pay(date(2018, 6, 28))
    e.pay(date(2018, 7, 28))
    e.pay(date(2018, 8, 28))
    assert e.total_pay() == 180000.0
...
def test_total_payroll_mixed2() -> None:
    my_corp = Company([SalariedEmployee(24, 'Gilbert the cat', 1200.0),
                       HourlyEmployee(25, 'Chairman Meow', 500.25, 1.0)])
    my_corp.pay_all(date(2018, 6, 28))
    my_corp.pay_all(date(2018, 7, 28))
    my_corp.pay_all(date(2018, 5, 28))
    assert my_corp.total_payroll() == 1800.75

def test_total_payroll_mixed2() -> None:
    my_corp = Company([SalariedEmployee(24, 'Gilbert the cat', 1200.0),
                       HourlyEmployee(25, 'Chairman Meow', 500.25, 3.0)])
    my_corp.pay_all(date(2018, 6, 28))
    my_corp.pay_all(date(2018, 7, 28))
    my_corp.pay_all(date(2018, 8, 28))
    assert my_corp.total_payroll() == 4802.25
