...
class PhoneLine:
    ...
    def make_call(self, call: Call) -> None:
        ...
        """
        # case if call in callhistory
        if (call.time.month, call.time.year) in self.callhistory.outgoing_calls:
            if call in (self.callhistory.
                        outgoing_calls[(call.time.month, call.time.year)]):
                return
        # case if call not in callhistory
        self.callhistory.register_outgoing_call(call)
        # bill it
        # bill call in contract(according to different contract)
        if (call.time.month, call.time.year) in self.bills:
            self.contract.bill_call(call)
        # initalize a bill with certain contract
        else:
    ...
    def receive_call(self, call: Call) -> None:
        ...
        """
        # case if call in callhistory
        if (call.time.month, call.time.year) in self.callhistory.incoming_calls:
            if call in (self.callhistory.
                        incoming_calls[(call.time.month, call.time.year)]):
                return
        # case if call not in callhistory
