...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    """
    # case empyt log(dictionary)
    if len(log) == 0:
        return
    # case empyt events(list)
    if len(log['events']) == 0:
        return
    ...
    for i in range(len(log['events'])):
        if log['events'][i]['type'] == 'call':
            scr_number = log['events'][i]['src_number']
            dst_number = log['events'][i]['dst_number']
            time = datetime.datetime.strptime(log['events'][i]['time'],
                                              "%Y-%m-%d %H:%M:%S")
            duration = log['events'][i]['duration']
            src_loc = tuple(log['events'][i]['src_loc'])
            dst_loc = tuple(log['events'][i]['dst_loc'])
            call = Call(scr_number, dst_number, time,
                        duration, src_loc, dst_loc)
    # records call object to right customer
            customer_call = find_customer_by_number(scr_number, customer_list)
            # find_customer_by_number will not return None
            # since the customer_List already contains all
            #  the customers from the log
            customer_call.make_call(call)
            customer_receive = find_customer_by_number(dst_number,
                                                       customer_list)
            customer_receive.receive_call(call)
    # every time a new month is detected for the current event(call or sms),
    # advance all customers to a new month of contract by
    # using the new_month() function
        # case call
            time = datetime.datetime.strptime(log['events'][i]['time'],
                                              "%Y-%m-%d %H:%M:%S")
            if time.month > billing_month or time.year > billing_year:
                new_month(customer_list, time.month, time.year)
                billing_month = time.month
                billing_year = time.year
        # case sms
        if log['events'][i]['type'] == 'sms':
            time = datetime.datetime.strptime(log['events'][i]['time'],
                                              "%Y-%m-%d %H:%M:%S")
            if time.month > billing_month or time.year > billing_year:
                new_month(customer_list, time.month, time.year)
                billing_month = time.month
                billing_year = time.year
        # now billing_month, billing year is the latest month year combination
